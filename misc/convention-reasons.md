...
- From that point on, we agreed that when a *5 Stall Setup Clue* was given, there was no need to actually *Finesse* or *Bluff* the card - it should always just blind-play automatically. Then, we formalized the rules around it, and made some arbitrary extensions (like the *5 Pull* still working in the same way, even if the 5 was not actually on *Finesse Position*).
