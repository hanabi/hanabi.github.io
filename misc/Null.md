...
* A **color** positional clue corresponds a slot number that is based on the order of the stacks. Stacks correspond to the slot numbers **from right to left**.
  ...
  * For example, in a 4-player game of the *Null (6 Suit)* variant, there are 5 color clues and 4 slots in the hand. Thus, a purple clue means to *Chop Move* (since there is no slot 5 card).
