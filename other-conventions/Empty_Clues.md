...
* In later American releases of the game, it is [the printed rules](https://github.com/Zamiell/hanabi-conventions/blob/master/other-conventions/Rules.txt) explicitly disallow empty clues.
...
* Unlike Hat-Guessing, it does not matter which player recieves a clue. If it is possible to give the empty clue to more than one player, then the clue should be given to the left-most player.
