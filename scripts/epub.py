...
def main():
    ...
    content_soup = update_content(
        content_soup, toc, linked_files, examples_paths, unneeded_parts
    )
        ...
        if "file" == node["type"]:
            ...
            for a_tag in converted_soup.find_all("a", href=True):
                ...
                if href.startswith("http://") or href.startswith("https://"):
                    ...
                    continue  # TODO: Delete and impl anchor links
...
def construct_xhtml(file_info, link_map):
    ...
    for tabs_open_tag, tabs_content, tabs_close_tag in TABS_P.findall(mdx_source):
        mdx_source = mdx_source.replace(tabs_open_tag, "", 1)
        for tab_open_tag, tab_content, tab_close_tag in TAB_P.findall(tabs_content):
    ...
    if len(mdx_matches) > len(svgs):
        raise Exception(
            f"Unexpected MDX tag found. Have {len(svgs)} SVGs to"
    ...
    for svg in svgs:
        ...
        invalid_tags = [
            tag
            for tag in svg.find_all(id=True)
...
def reset_ref_root(ref, from_doc_path):
    ...
    while True:
        ...
        if ref.startswith("https://") or ref.startswith("http://"):
            ...
            back_c = ref.count("../")
            root_ref = "/".join(parent_path_parts[:-back_c] + [ref[back_c * 3 :]])
        elif ref.startswith("/"):
            root_ref = ref[1:]
        else:
            root_ref = "/".join(parent_path_parts + [ref])
...
def update_content(content_soup, toc, linked_files, examples_paths, unneeded_items):
    ...
    for path in examples_paths:
        ...
        if file_t not in media_type_map:
            raise Exception(
                f"Found unexpected file type {file_t}. Expected"
...
def construct_navpoint_tree(toc, play_order_start=1, depth=1):
    ...
    for item in toc:
        if "file" == item["type"]:
            soup.append(
                create_ncx_navpoint(
                    play_order, item["title"], f'parts/{item["id"]}.xhtml'
                )
            )
            play_order += 1
        elif "folder" == item["type"]:
            ...
            navpoint = create_ncx_navpoint(
                before_play_order, item["name"], first_content_src, True
            )
...
def create_ncx_navpoint(play_order, label, content_src, is_parent=False):
    ...
    navpoint = soup.new_tag("navPoint", id=xmlified_id, playOrder=str(play_order))
