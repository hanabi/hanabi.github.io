...
import TempoClueChopMoveComplicated from "./level-6/tempo-clue-chop-move-complicated.yml";
...
- _Valuable Tempo Clues_ can never cause a _Tempo Clue Chop Move_. However, if it is ambiguous as to whether the clue was _Valuable_, then players should choose the more conservative option.
- For example, in a 3-player game:
  ...
  - Bob has:
    ...
    - a red 4 on slot 5 with a number 4 clue on it
  - Alice clues red to Bob, which re-touches his slot 4 card and his slot 5 card, "filling" both of them in.
  - Bob knows that this is a _Tempo Clue_ on the red 2. Furthermore, Bob knows that since the _Tempo Clue_ only "got" one card, it is not a _Valuable Tempo Clue_. Therefore, it must be a _Tempo Clue Chop Move_ as well, so Bob marks his slot 3 as _Chop Moved_.
  - Bob plays the red 2.
  - Cathy has to perform a similar analysis to Bob, but it is a bit harder. From Cathy's perspective, there are two possibilities for Alice's clue:
    ...
    - Second, it could be a _Tempo Clue_ on the red 2 + a _Finesse_ on the red 3 + a _Tempo Clue_ on the red 4.
