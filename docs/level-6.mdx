...
- We agree that _Tempo Clues_ of this nature cause a _Chop Move_ in addition to getting the focus of the clue to play.
...
- In the _Early Game_, players must exhaust all _Play Clues_ and _Save Clues_ that satisfy _Minimum Clue Value Principle_. Since clues to playable _Chop Moved_ cards count as normal _Play Clues_, it follows that players are obligated to give these kinds of clues before ending the _Early Game_.
...
- Oftentimes, either you or the player who comes after you can perform a clue. And the clue would have the same result if done by either player. So who should do it?
...
- This concept applies beyond _Finesses_; it should be applied to any complicated clue. We formalize this as the _Clarity Principle_: if two lines would result in identical outcomes, always prefer the line with the least amount of complexity.

### Clarity Principle (Part 2)

- In the previous section, we showed how to use _Clarity Principal_ to decide between two lines with two identical outcomes: prefer the less complicated line.
- But _Clarity Principal_ can apply EVEN IF one line results in a better game outcome. That might seem counterintuitive, so read on!
...
- Often, these kinds of complicated clues end up in misplays and lost games. And in the post-game review, the person who gave the clue can get defensive: "If you guys played perfectly, then my clue would have worked!"
...
- But things are different when Alice gives a really complicated clue that Bob should _technically_ be able to figure out, but doesn't. In this case, Bob shares a little of the blame. **But it is mostly Alice that is at fault**.
