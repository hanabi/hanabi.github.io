...
- Formally, a "no information" clue is defined by having the following criteria:
  - The re-clue gave no additional positive or negative information to the card.
  - The elements in the superposition on the re-clued card were the same before and after the clue sequence.
- Since "no information" clues are not very valuable, we agree that a "no information" clue on a two-away-from-playable card **must** "get" at least 2 new cards (as opposed to a single _Finesse_ that only gets 1 new card).
- For example, in a 3-player game:
...
- Even if a clue does not "fill-in" a card, it not necessarily a "no-information" clue.
- For example, in a 3-player game:
  - It is the first turn of the game.
  - Alice clues red to Cathy, touching a card on slot 1 as a _Play Clue_.
  - Bob blind-plays a blue 1.
  - Cathy marks her red card as either the red 2 or the red 3.
  - Later on the game, red 1 is now played.
  - Alice clues red to Cathy, re-touching the card that is in a superposition of red 2 or red 3.
  - Bob blind-plays a blue 2.
  - Cathy updates the note on her red card to be exactly red 3.
  - Everyone at the table clearly understands that this clue gave valuable information to Cathy's red card and would not be considered a "no information" clue.
...
- Usually, a "no-information" clue given to a two-away-from-playable card conveys a _Double Finesse_. But you can also use it to perform a _Double Bluff_. For example, in a 3-player game:
  - Nothing is played on the stacks.
  - Alice clues 3's to Cathy, which touches one 3, the red 3.
  - Bob blind-plays blue 1. At this point, everyone knows that Alice has performed a _3 Bluff_, and from Cathy's perspective, her 3 can be any 3 at all.
  - Cathy discards.
