...
- Less commonly, players will see the next card in someone else's hand, but it will not be on _Finesse Position_. In these cases, it might still be good to play the card without _Priority_, just so that the player with the next card will have something to do on their turn.
