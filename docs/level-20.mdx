...
- However, playing a known card instead of an unknown card can trigger a _Priority Prompt_ / _Priority Finesse_ - but only if the known card has lower priority than **all possible cards** in the unknown superposition.
- For example, in a 3-player game:
  ...
  - Alice is going to play a card on her turn. She has the choice between two playable cards:
    ...
    - A blue 2 in slot 3. From Alice's perspective, it is in a superposition of `blue 2, yellow 2`.
  - Alice plays the red 2.
  - Bob needs to determine if the red 2 had _Priority_ so that he can react accordingly.
  - Bob knows that the known card must have lower priority than all unknown card superpositions for _Priority_ to take effect.
  - Bob can see that both blue 2 and yellow 2 have priority over red 2 (unknown identity before left-most). Therefore, because red 2 has strictly lower priority than the unknown card, Alice playing red 2 triggered _Priority_. Bob plays his _Finesse Position_ as a _Priority Finesse_, and successfully plays red 3.
- For example, in a 3-player rainbow game:
  ...
  - Alice plays the red 2.
  - Bob needs to determine if the red 2 had _Priority_ so that he can react accordingly.
  - Bob checks whether the known card has lower priority than all unknown card superpositions.
  - Bob can see that red 2 has _Priority_ over rainbow 3 (red 2 is lower rank). Because red 2 was not strictly lower priority than Alice's unknown card, Alice playing red 2 did not trigger _Priority_. Bob should not react.
