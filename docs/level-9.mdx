...
import DoubleDiscardSituation1 from "./level-9/double-discard-situation1.yml";
import DoubleDiscardSituation2 from "./level-9/double-discard-situation2.yml";
...
- First, see the section on [Minimum Clue Value Principle](beginner/minimum-clue-value-principle.mdx), one of our core principles.
- On most turns, players will not have any known-playable (or known-trash) cards. Thus, on most turns, players will either give a clue (if it is a good clue) or discard their chop card (the null action).
- However, in certain situations, discarding the chop card can be dangerous, having the potential to lower the maximum score. In those situations, the only safe action to perform is to give a clue. So, in those situations, we allow players to give low-value clues that would otherwise be illegal.
- We call these types of situations _Stalling Situations_. The rest of this page will go over the specific kinds of _Stalling Situations_ that exist.

### Allowable Stall Clues (Stall Table)

- In _Stalling Situations_, everyone understands that a player might be giving a low-value clue. However, there are limits - they are not allowed to just clue anything they want. These are the specific clues that they are allowed to give along with the precedence of each clue:
...
| 4 (tied)   | _Locked Hand Save_<br />(saving any card on chop, as long as doing so would not _Lock_ the other player) | ❌                            | ❌                      | ❌                                           | ✔️                       | ❌                   | use context           |
...
- Don't let this complicated table scare you: it is mostly describing some common-sense Hanabi intuitions.
- For example:
  ...
  - Alice does have a _Tempo Clue_ to give (precedence 3), but _Tempo Clue Stalls_ are not valid in the _Early Game_. Thus, if Alice gives a _Tempo Clue_, it would instead be interpreted as a _Tempo Clue Chop Move_.
...
- The _Early Game_ is introduced in the [beginner's guide](beginner/early-game.mdx). In short:
  - The _Early Game_ is defined as the period of time before any player has discarded their chop card.
  - Players **must** "extinguish" all of the available _Play Clues_ and _Save Clues_ before ending the _Early Game_.
...
- For level 11 players, note that _Permission to Discard_ does not apply for Bob when Alice was blind-playing a card in a situation where [_Guide Principle_](level-11.mdx#guide-principle) applies.

### Double Discard Situations / Double Discard Avoidance (DDA) (Severity 2 Stalling)

- Oftentimes in Hanabi, two players will discard in a row. However, in rare cases, this can cause a perfect score to be lost.
- For example, in a 3-player game:
  - Alice has nothing to do. Alice discards her chop and it is a red 4.
  - Bob has nothing to do. Bob discards his chop and it is the other copy of the red 4.
...
- More formally, a _Double Discard Situation_ is defined as when:
  ...
  1. **and** if the current player discarded, they would discard the last copy of that card.
...
- Generally speaking, it is a bad situation when someone's hand gets fully clued (or fully _Chop Moved_). This is known as being _Locked_ and it should be avoided if possible. However, sometimes it cannot be helped, like when a player draws three 5's in a row.
...
- As mentioned in [level 2](level-2.mdx#the-5-stall-cluing-off-chop-5s), _5 Stalls_ are when a player clues an off chop 5 purely because they want (or need) to give a stall clue.
- _5 Stalls_ are usually performed in the _Early Game_, but they can also be performed in the _Mid-Game_ if a player happens to be in a special "stalling" situation where they are not allowed to discard.
- This section adds some extra rules that apply to the _5 Stall_. Since playing with these extra rules makes things more complicated, we wait until later levels to introduce them.

#### Early Game 5 Stalls

- In the _Early Game_, the team must collectively give at least one _5 Stall_ (if possible). (See the [previous section](#extinguishing-every-clue) that goes over the _Early Game_ rules.)
- In the _Early Game_, the team can give as many _5 Stalls_ as they want before initiating the _Mid-Game_.

#### 5 Stalls Closest to Chop
...
#### 5 Stalls to the Previous Player

- The section on [_Permission to Discard_](#permission-to-discard-ptd) applies to _5 Stalls_. This means that if the previous player gave you "permission to discard", you do not have to perform a _5 Stall_ on the previous player, even if it is the _Early Game_ and the team has collectively not performed any _5 Stalls_ yet.
  - Note that _Permission to Discard_ affects **every** kind of clue, not just _5 Stalls_. But we explicitly clarify the interaction here so that it is easier to discover when reading over the various _5 Stall_ exceptions.
...
- The _Finesse Position Exception_ **only applies** when the playable card on _Finesse Position_ can actually be _Finessed_ or "gotten" by someone else. For example, it would not apply in a situation like this:
  ...
  - Alice sees that Cathy has a unclued playable card on her _Finesse Position_.
...
- A better way to signal this information is to deliberately leave them at 0 clues. By convention, this means that one of their cards is actually playable. The player should play the card in their hand that is most likely to be playable. If there is more than one option, then they should know that the leftmost is playable. (This follows from _Leftmost Playable Principle_.)
...
- An _8 Clue Save_ can be performed on **any** card, not just a critical or playable card. However, players can only do an _8 Clue Save_ if there are no higher precedence clues available. See the "Allowable Stall Clues" section above.
...
- The _8 Clue Save_ does not apply on the first turn of the game.
