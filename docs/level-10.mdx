...
- The duplicated card in your own hand must be unknown when given, otherwise the clue receiver will simply think it's the next card in the suit. 
  - Due to _[Information Lock](level-3.mdx#information-lock-principle)_ this will be hard fix, discarding the known card in your hand will be interpreted as a _Gentleman's Discard_ instead of a _[Sarcastic Discard](level-3.mdx#the-sarcastic-discard)_.
