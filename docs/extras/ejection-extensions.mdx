...
These are additional rules that can apply to _._, _Discharges_, and _Charms_.
...
- Note that you are not allowed to perform _Out-of-Position ._ in reverse, unless it is super-obvious what is going on.
- Remember that just because _OOP ._ are technically legal in some situations, this does **not** mean that players should use them. Just like any other complicated move, only use _OOP ._ when it is super-clear to the rest of the team what is going on. Otherwise, a simpler line should be chosen.
...
- Similarly, since _._ follow most of the same rules as _Bluffs_, you are not allowed to perform an _Ejection_ on a player that is already "loaded" with a blind-play for a _Finesse_.
...
- For example, in a 4-player game:
  ...
  - Normally, _Ejection_ calls for the _Second Finesse Position_, but in this case, Cathy knows that _Stacked 5 Color ._ call for the _First Finesse Position_ instead of the _Second Finesse Position_, so Cathy blind-plays slot 2 (instead of slot 3).

<StackedEjection />

- It is only legal to perform _Stacked Ejection_ on a player if they are _Finessed_ for exactly one card. Otherwise, the clue would be considered undefined.
