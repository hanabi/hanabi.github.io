...
- Note that a _Double Ignition_ takes precedence over a _Null Kick_; if Bob sees that someone else has a playable card on _Finesse Position_, that interpretation should be preferred.
...
- When Alice gives a number 5 clue that is not a _5 Save_ or a _5's Chop Move_, **as long as Bob is not loaded**, it signals a _Null 5's Chop Move_ on Bob.
  - Note that the number 5 clue can be given to any player; it does not necessarily have to be on Bob.
- The _Null 5's Chop Move_ is **only** allowed to save null cards that are playable / phantom playable or critical (or a null 2 that isn't visible elsewhere, same as an ordinary _2 Save_).
- This convention applies only to Bob; see _The Double Save_ for saving other players.
- For example, in a 4-player game of the "Null (5 Suits)" variant:
  - It is the first turn of the game.
  - Bob has a null 2 on chop, and there are no other null 2s to be seen.
  - Alice cannot save the null 2 with a clue, but fortunately, Cathy has a red 5 in slot 2.
  - Alice clues number 5 to Cathy as a _Null 5's Chop Move_ to _Chop Move_ the null 2.
  - Cathy knows that this was a _Null 5's Chop Move_ and saves her 5 for later.
...
- For example, in a 4-player game of the "Null (5 Suits)" variant:
  - Bob has a globally known playable green 1 on slot 1, so he is **loaded**.
  - Bob has a null 1 on chop, and Alice has an unclued red 5 on slot 2.
  - Alice clues number 5 to Cathy as a _Loaded 5 Null Play Clue_.
  - Bob blind-plays his rightmost unclued card. It is a null 1.
  - Cathy knows that this was a _Loaded 5 Null Play Clue_ and saves her 5 for later.
...
- Given that the _Null 5's Chop Move_ only applies to Bob and only _Chop Moves_ one card, it can be difficult to save two critical cards in a row on Bob.
- A _Double Save Situation_ occurs when:
  ...
  4. Cathy's two critical cards cannot be saved with one clue (e.g. a _5's Chop Move_, a _Trash Chop Move_, or an _Unnecessary Reverse Trash Finesse_).

#### The Double Save

- In the _Double Save_, Alice gives a number 5 clue, similar to a _Null 5's Chop Move_. But Bob does not _Chop Move_, and instead immediately performs a second _Save Clue_ or _Chop Move_ on Cathy.
- _Double Saves_ can only be performed in _Double Save Situation_.
- When a _Double Save_ interpretation is possible, Cathy should always assume that a _Double Save_ has occurred. In other words, Cathy should not assume that Alice performed a _Null 5's Chop Move_ on Bob and that Bob gave her a _Play Clue_.
...
- For example, in a 3-player game of the "Null (5 Suits)" variant:
  ...
  - To save these cards, Alice initiates a _Double Save_ by cluing number 5 clue to Cathy, which forces Bob to clue a number 4 to Cathy.
  ...
  - Normally, since Cathy sees that Bob has a playable null 1 on chop, she would consider this to be a _Loaded 5 Null Play Clue_. However, Cathy knows that _Double Save_ situations take precedence over _Loaded 5 Null Play Clues_. She therefore _Chop Moves_ her card on slot 5.

<GoodDoubleSave />

#### The Double Save Corollary

- Since Cathy should always assume a _Double Save_ occurred whenever it is possible, Alice must take care not to create ambiguous situations by performing a _Null 5's Chop Move_ at the wrong time.
- For example, in a 3-player game of the "Null (5 Suits)" variant:
  ...
  - This is because using a _Null 5's Chop Move_ on Bob would prevent Bob from doing a _Null 5's Chop Move_ on Cathy, since it would look like a _Double Save_.
...
- For example, in a 3-player game of the "Null (5 Suits)" variant:
  ...
  - If Alice clues number 5 to Bob as a _Null 5's Chop Move_, Bob will be unable to get the yellow 2 to play.
