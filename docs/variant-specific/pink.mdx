...
- To summarize: when looking for a pink card, look for it in leftmost clued known pink, then leftmost clued matching rank, then leftmost unclued possible true (_Finesse_), then leftmost clued mismatched rank, then leftmost unclued (_Known Bluff_).
- For example, in a 3-player game:
