...
- The _Pink Promise_ convention does **not** change how _Prompts_ work - players are still expected to play the leftmost card into a _Prompt_ (or a _Priority Prompt_), just like they would in any other variant.
- For example, in a 3-player game:
  ...
  - Bob knows that he is _Prompted_ for the red 2. Bob has three cards in his hand marked with a number 2 clue. Even though Bob knows for sure that slot 5 is a 2, Bob knows that the _Prompt_ simply calls for the leftmost card.
...
- This rule does **not** apply to potential pink cards that only have a single rank clue on them when the rank on them does not match the rank of the promised card. In this situation, players should prefer playing their _Finesse Position_ (if it could match) instead of playing the clued card.
- To summarize: when looking for a pink card, look for it in leftmost clued known pink, then leftmost clued matching rank, then leftmost unclued possible (_Finesse_), then leftmost clued mismatched rank, then leftmost unclued (_Known Bluff_).
- For example, in a 3-player game:
  - All of the 2's are played on the stacks.
  - Alice clues pink to Cathy, touching a pink 4 on slot 2 as a _Play Clue_.
...
- This rule does not apply when a card has a superposition that specifically includes the prompting card, and that superposition was from a special move (such as a _4 Charm_ or a _Bubblegum Bluff_).

### Remaining 1's After All the 1's Have Been Played

- Oftentimes a player will receive a number 1 clue that touches a bunch of 1's. In these situations, they will play them all from oldest to newest (using the _1's Assumption_).
...
- For example, in a 3-player game:
  ...
  - Cathy discards.
  - Alice discards.
  - Bob plays slot 3, and it is a blue 1.
  - Cathy discards.
...
- In this situation, it must not be a _Play Clue_ at all. Instead, it is a _Fix Clue_ - the rank of the leftmost pink card must match the clue.
- For example, in a 3-player game:
  ...
  - Bob instead knows that this must be a _Pink Fix Clue_ and that his leftmost pink card (on slot 4) must be exactly the pink 3.
...
- When one or more pink cards are retouched with a color clue, the interpretation should be as normal: play the leftmost card as a _Tempo Clue_.
...
- For example, in a 3-player game:
  ...
  - Bob knows that since all of the 3's are already played, this might be a _Trash Chop Move_. If this is the case, he should discard slot 3, and then chop move slot 4 and slot 5.
...
- Normally, you can only _Prompt_ a card if it is the leftmost clued card. However, you can _Prompt_ a non-leftmost clued card by violating _Pink Promise_. This is called a _Bubblegum Prompt_, which is conceptually similar to a _Bubblegum Finesse_.
- For example, in a 4-player game:
  - All the 1's are played on the stacks.
...
- The two-away-from-playable form of the _Bubblegum Bluff_ can be hard in the same way as the _Hard 3 Bluff_.
  - Because of this, they are always a _Known Bluff_ instead of a _Bubblegum Finesse_.
- For example, in a 3-player game:
  - It is the first turn and nothing is played on the stacks.
  ...
  - Thus, Bob concludes this is a _Bubblegum Bluff_, with a playable card in slot 2.
...
- For example, in a 4-player game:
  - It is the first turn and nothing is played on the stacks.
  - Alice clues number 3 to Donald, touching a pink 4 on slot 2.
  ...
  - From Donald's perspective, he knows that Alice performed a _Bubblegum Bluff_. Donald marks his slot 2 card as a pink 2 (i.e. the _one-away-from-playable_ pink card).
...
- The _Bubblegum Double Bluff_ can be hard in the same way as the _Hard Double Bluff_.
  For example, in a 4-player game:
  - It is the first turn and nothing is played on the stacks.
  ...
  - Cathy determines this is a _Bubblegum Double Bluff_ and plays her slot 1. It is a pink 2 and it successfully plays on the stacks.
  - Donald knows their clued card is a pink 4. Bob and Cathy also know this.
  - The team is re-synchronized on the identity of the focus the _Bubblegum_ clue and no further blind plays are required.
...
- _Turnabout Ejections_ explicitly have precedence over all other conventions, including _5 Number Ejections_. In other words, if a clue could be a _Turnabout Ejection_, then players are supposed to assume that it is.
...
- The _Bubblegum_ conventions usually take precedence over other kinds of moves. Specifically:
  ...
  - The _Bubblegum_ conventions take precedence over an _Unknown Trash Discharge_ and an _Unknown Dupe Discharge_.
...
- When a player performs a _5 Stall_ that touches two or more 5's, the focus of the clue is not obvious.
- We define the focus of a _5 Stall_ to be the specific 5 that is closest to the chop.
...
- For example, in a 3-player game:
  ...
  - Because of the _Pink Promise with a 5 Stall_ convention, Bob knows that since this clue is focused on slot 2.
...
- When a player performs a _5's Chop Move_ that touches two or more 5's, the focus of the clue is not obvious.
- We define the focus of a _5's Chop Move_ to be the specific 5 that is one-away from the chop. (This is similar to how _Pink Promise_ works with a _5 Stall_.)
...
- For example, in a 3-player game:
  ...
  - Because of the _Pink Promise with a 5's Chop Move_ convention, Bob knows that since this clue caused him to _Chop Move_, it is focused on slot 4.
...
- When a player performs a _5 Pull_ that touches two or more 5's, the focus of the clue is not obvious.
- We define the focus of a _5 Pull_ to be the 5 that is directly to the left of the "pulled" card.
...
- For example, in a 3-player game:
  ...
  - Because of the _Pink Promise with a 5 Pull_ convention, Bob knows that since this clue was a _5 Pull_, it must be focused on slot 2 (the card that was next to the pulled card).
  - Thus, _Pink Promise_ applies to this card, so he marks slot 2 as a 5.
  - Later on in the game, another rank clue "fills in" Bob's slot 2 card to be a pink card. From _Pink Promise_, Bob knows that it must be exactly the pink 5.
