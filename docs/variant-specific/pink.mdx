...
- Rarely, it is also possible to lie about the rank of a pink card that is not yet playable. Doing so communicates the slot number of where the connecting card is to other players on the team.
- By unexpectedly playing cards from a matching slot number, the team can communicate the true rank of the pink card, fixing the lie.
...
- _Bubblegum Finesses_ can only be used to get pink cards.
  - In other words, if a pink card plays in response to a _Bubblegum_ move, it is guaranteed to the team to be a _Finesse_.

### The Bubblegum Prompt

- Similar to a _Bubblegum Finesse_, it is possible to violate _Pink Promise_ to prompt "unpromptable" pink cards.
- For example, in a 4-player game:
  - All the 1's are played on the stacks.
  - Bob has a card with negative pink on finesse and 3 cards marked with 5 in slots 2-4.
  - Alice clues Donald number 3, which focuses a pink 4 on slot 2 and touches a pink 3 on slot 3.
  - Bob sees that Cathy does not have the pink 2, so he plays his middle "5". It is the pink 2.
  - Donald knows that the only explanation for this play is a _Bubblegum Prompt_ on the pink 4.
  - Donald also sees that neither Bob nor Cathy has a pink 3. The only other card that could be the pink 3 is on his own slot 3.
  - Cathy sees that Donald knows the identity of the focused pink card, and sees a pink 3 in Donald's hand. She does something unrelated.
  - Donald plays the pink 3 from slot 3.
- It is possible for a _Bubblegum Prompt_ to have a _Finesse_ component.
  - Modifying the above example, imagine Donald only has a single pink card in slot 2: a pink 4 clued with the number 3.
  - On Cathy's turn, she does not see the pink 3 in Bob's hand. Since Alice promised that the pink 4 is playable, Cathy must have the pink 3.
  - Cathy plays her finesse position, and it is the red 2. Because Alice gave a _Finesse_, Cathy knows that the pink 3 must be _Layered_.
...
- Note that, as with a standard _Bluff_, the receiver of a _[Bubblegum Bluff](#the-bubblegum-bluff)_ marks their pink card the lowest possible value it could be.
- However, it is sometimes possible that the lowest-possible value a card could be is actually _two-away-from-playable_.
- For example, in a 3-player game:
  - It is the first turn and nothing is played on the stacks.
  ...
  - Bob concludes this is a _Bubblegum Double Finesse_, with pink 1 in slot 2 and pink 2 in slot 1.
  - Bob blind-plays his slot 2, but it is a red 1. It is now revealed to Bob that this was a _Bubblegum Bluff_ instead of a _Bubblegum Double Finesse_.
...
- This used to be called a _Taffy Bluff_, but the term was phased out because the logical distinction between _Taffy_ and _Bubblegum_ was minor.

### The Bubblegum Double Bluff

- Normally, after a _Bubblegum Bluff_, the entire team is synchronized on the identity of focus of the _Bubblegum_ move.
- But what if the note the clue receiver would write on their card is still too small?
...
- For example, in a 4-player game:
  ...
  - Bob blind-plays slot 3, and it is a red 1.
  - From Donald's perspective, he knows that Alice performed a _Bubblegum Bluff_. Donald marks his slot 1 card as a pink 2 (the lowest-possible value, in this case _one-away-from-playable_).
  ...
  - Donald realizes that if his clued card were a pink 2, then Cathy would not have done anything. Donald also knows that if the card were a pink 3, Bob would have played slot 1 as a _Finesse_ or _3 Bluff_. Thus, Donald updates the note on his card to be a pink 4.
- This used to be called a _Taffy Double Bluff_, but the term was phased out because the logical distinction between _Taffy_ and _Bubblegum_ was minor.
...
- The _Bubblegum Bluff_ conventions usually take precedence over other kinds of moves. Specifically:
