...
- Similar to brown cards, if a player gives a _Tempo Clue_ to a single muddy or cocoa rainbow card, it is never a _Tempo Clue Chop Move_. If a player gives a _Tempo Clue_ to multiple muddy or cocoa rainbow cards, it means only that the right-most muddy or cocoa rainbow card is playable.
- This is because it is fairly valuable to get muddy and cocoa rainbow cards out of the hand as soon as possible in case blocking cards are drawn.
...
- In variants with a pink suit, the _[Pink Choice Tempo Clue](pink.mdx#the-pink-choice-tempo-clue)_ convention is "turned on", since players often have a _Free Choice_ with how they can clue pink cards. Muddy Rainbow and cocoa rainbow have a similar convention.
- When one or more muddy or cocoa rainbow cards are retouched with a color clue, and there are no "new" cards introduced (or, if the only "new" cards introduced are trash), then extra information can be conveyed by what color is chosen. The color chosen should correspond to the slot that they should play.
- This is called a _Muddy Rainbow Choice Tempo Clue_, or just a _Mud Clue_ for short.
- The position of the muddy or cocoa rainbow card, counted **from oldest to newest** (right to left), corresponds to the ordering of the colors **from right to left**, skipping the cards that are not touched by the color clue (and the known unplayable cards). Furthermore, colors always "wrap around" to the oldest card.
