...
import FixClue from "./reversed/fix-clue.yml";
...
- Even though the reversed 5 is playable from the start, it cannot be _Play Clued_ using a 5 clue. If players want to get a 5 to play, they must clue it with color. (This convention changes in _[Reversed White](#reversed-white)_.)
...
- If there are two or more cards are touched from _1's Chop Move_, the reversed 1 is promised on the rightmost.
...
This convention applies to any variant with a white reversed suit:

### The White 5 Play Clue

- As long as the reversed white 5 is not yet played, all _5 Stalls_, _5's Chop Moves_, and _5 Pulls_ are "turned off".
- Instead, any number 5 clue that is not focused on the chop will just be a simple _Play Clue_ on a reversed white 5.
