...
- In this variant, it is possible to have 1's in a player's hand that are "misranked".
- _Finesses_ in this variant work the same as in a regular pink variant: play _Finesse Position_ (if it could match) instead of a potentially misranked 1.

## Omni Ones

- Omni Ones inherit all the conventions from Pink Ones.
...
- We arbitrarily choose red since it is the leftmost color.
...
- For example, in 3-player game:
  ...
  - Bob knows that this is a _Color Promise Finesse_ and blind-plays his _Finesse Position_. It is a red 1 and it successfully plays.
...
- For example, in a 3-player game:
  ...
  - Bob knows that normally, _Prompts_ are supposed to take precedence over _Finesses_. However, in this case, since his green card only has a single color clue on it, the _Color Prompt Exception_ applies, so he should assume a _Finesse_.
