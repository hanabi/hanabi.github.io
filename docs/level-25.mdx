...
- If **known trash** is used as clue target, the "extra" thing is an _Ignition_. The _Ignition_ is usually on Bob, with two exceptions:
  - If Bob can already play a card on his turn and sees a playable card on _Finesse Position_ or
  - If Bob was the receiver of the clue and is blind-playing from the clue, then the _Ignition_ is on the last player with a playable card on _Finesse Position_ (similar to a _Double Ignition_).
