...
- A _Double Discard situation_ is one where
...
- If the team is unlucky, this could cause the team to lose the game. So, we agree to treat this as a "stalling" situation as well.
...
- There are some situations in which the team might need to "stall" by giving a low-value clue.
- In order of severity, such situations include:
  ...
  - Severity 2: _Double Discard situations_ and _Scream Discards_ were defined above.
  ...
  - Severity 4: The rules of the game prohibit discarding if the team already has 8 clues. In this case, a player with no cards to play might be forced to stall instead.
...
- The types of stalls covered in the table are summarized below:
  ...
  - At Severity 2 or higher, as a last resort, the player can _Hard Burn_ by spending a clue to provide no new information (e.g. re-cluing a known unplayable 5).
- In the _End-Game_, stalling is often necessary in order to win the game, since discarding with too few cards in the deck can cause the game to end before all cards could be played. Because _End-Game_ strategy is so complicated, the stalling table does not prescribe conventions for what is and isn't legal, and players need to instead use context and common sense.
