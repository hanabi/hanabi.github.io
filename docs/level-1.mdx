...
- _Playable_ is a property of a card. To start with, see the section on [_Delayed Play Clues_](beginner/delayed-play-clues.mdx) from the beginner's guide.
...
- Trash is a property of a card. A card is considered trash if it can never play on the stacks or if two or more copies of the card are currently _Clued_.
- Since we play with _Good Touch Principle_, we intentionally avoid touching trash cards. However, at later levels, it can be sometimes advantageous to touch trash cards in specific situations.
- In rare cases, it can be possible for a card to be both _playable_ and trash at the same time.
...
- See the section on [_Good Touch Principle_](beginner/good-touch-principle.mdx) from the beginner's guide.

### Save Principle

- See the section on [_Save Principle_](beginner/save-principle.mdx) from the beginner's guide.

### Minimum Clue Value Principle

- See the section on [_Minimum Clue Value Principle_](beginner/minimum-clue-value-principle.mdx) from the beginner's guide.
...
- For level 9 players, there are [some additional rules](level-9.mdx#the-early-game-severity-1-stalling) about the _Early Game_. (Don't worry about them for now if you are only level 1.)
...
- By definition, you can only perform a _2 Save_ with a number clue. (If a color clue was used to touch an unclued 2, then it would be a _Play Clue_ instead.)
  - The exception is if the other copy of the 2 is in the discard pile. Then, touching the card with a color clue would be a _Critical Save_. But that would not be classified as a _2 Save_.
...
- Then Alice should always play the clued card.
