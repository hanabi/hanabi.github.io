...
import SelfBluff from "./level-11/self-bluff.yml";
...
- _Finesses_ are quite efficient. So, when you see a playable card in someone's _Finesse Position_ slot, you will often want to "get" the card with a _Finesse_ (instead of cluing it directly). However, oftentimes, the proper "connecting" card is not on the table.
- In this situation, players can perform an alternate strategy. By cluing an unrelated, _one-away-from-playable_ card, the next player will think that it is a _Finesse_ and that they have the connecting card. Then, they will blind-play their _Finesse Position_ card. This is called a _Bluff_.
...
- Importantly, **_Bluffs_ are only allowed if the very next player will blind-play on the very next turn.**
...
- _Self-Bluffs_ must be given with rank clues, as a _Self-Bluff_ with a color clue would be nonsensical.
  - For max-level players, see the section on the [Self Color Bluff](extras/special-bluffs.mdx#self-color-bluffs-1-for-1-form-scb).
- For example, in a 3-player game:
