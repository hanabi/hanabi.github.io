---
title: Level 23 - Charms & Miscellaneous
---

import FourCharm from "./level-23/4-charm.yml";
import BlazeDiscard from "./level-23/blaze-discard.yml";
import BlazeDiscard2 from "./level-23/blaze-discard-2.yml";
import HesitationBlindPlay from "./level-23/hesitation-blind-play.yml";

## Conventions

### Charms

- _Charm_ is similar to _Ejection_ and _Discharge_, except the _Fourth Finesse Position_ card is blind-played.
...
- When a _Play Clue_ is given to a previously untouched 4 that is not yet playable, Bob must react:
  - The first interpretation is that it is a _Prompt_, but if Bob has no matching cards in his hand, then a _Prompt_ is impossible.
  ...
  - The third interpretation is that it is a _Critical Color Bluff_, but this would not be the case if the touched 4 is not critical.
  - The fourth interpretation is that it is a _4 Double Bluff_, but if Bob sees that Cathy does not have a playable card on her _Finesse Position_, then a _4 Double Bluff_ is impossible (or very unlikely).
...
- For example, in a 4-player game:
  ...
  - Bob sees that Cathy's _Finesse Position_ card is unplayable, so a _4 Double Bluff_ is impossible.
...
- Some corner cases about the interaction between _Double Bluffs_ and _Charms_ when Cathy holds a one-away-from-playable card are spelled out in the [extras section](extras/charms.mdx#interaction-between-the-4-charm-and-double-bluffs-where-cathy-has-a-one-away-from-playable-card).
...
- For example, in a 3-player game:
  ...
  - Bob sees that Cathy's hand is completely unclued and is, from newest to oldest: `red 1 (trash), red 2, green 1 (trash), green 1 (trash), blue 1 (trash)`
...
- Since there are three copies of 1s, it can sometimes be ambiguous as to whether a discard of a playable 1 is a _Gentleman's Discard_ or a _Blaze Discard_. In this situation, _Blaze Discards_ take precedence. (This is because it is unlikely that Bob would have the third copy of of the 1 on his _Finesse Position_.)
...
- For example, in a 3-player game:
  ...
  - Bob knows that Alice's clue could be a direct _Play Clue_ on a yellow 3, green 3, blue 3, or purple 3. However, it could also be a red 3 if Alice intends a _Reverse Finesse_ through Cathy's red 2, so Bob must allow Cathy a chance to prove whether that is the case.
