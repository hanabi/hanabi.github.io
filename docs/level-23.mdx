...
- _Blaze Discards_ take precedence over a possible _Gentleman's Discard_ for a 1 on Bob.
  - For example, if Alice discards a known 1 and Bob sees that the second copy is not on _Finesse Position_, Bob should _Blaze_. (It is unlikely that the third copy is exactly on Bob's _Finesse Position_.)
