...
- If a player has a known-playable card and multiple known-trash cards, then discarding a trash card other than the expected trash card causes a skip in exactly the same way as a Trash Order Chop Move.
  - Recall that the discard order for trash is left-most first, except with trash that stems from an _Unknown Trash Discharge_ or _Unknown Dupe Discharge_, in which case we discard focus-first.
