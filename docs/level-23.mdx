...
- Note that it is explicitly illegal to perform a _Shout Discard Order Chop Move_ using a card that was touched by an _Unknown Dupe Discharge_. In other words, players must always discard the focus of an _Unknown Dupe Discharge_ first. If a player discards the cards out of order, it is to be treated as a known mistake.
