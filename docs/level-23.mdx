...
- If a player has a known-playable card and multiple known-trash cards, then discarding a trash card other than the left-most trash card causes a skip in exactly the same way as a _[Trash Order Chop Moves](level-14.mdx#the-trash-order-chop-move-tocm)_.
- For example:
