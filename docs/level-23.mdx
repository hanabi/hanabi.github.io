...
- Note that it is explicitly illegal to perform a _Shout Discard Order Chop Move_ if the player has exactly two trash cards that were touched by an _Unknown Dupe Discharge_. In the case, if a player discards the cards out of order, it is to be treated as a known mistake.
