...
import PlayingMultiple1s4 from "./level-3/playing-multiple-1s-4.yml";
...
import FixClue5 from "./level-3/fix-clue-5.yml";
...
- In this situation, we call the non-starting hand 1s "fresh" 1s. And it makes sense that fresh 1s would be more important than the non-fresh 1s, because otherwise they might have been clued earlier.
...
- If there are multiple freshly drawn 1's, they should be played newest to oldest, as is normal for play clues.
- For example, in a 2-player game:
  - Red 3 and blue 3 are played on the stacks.
  - None of the cards in Bob's hand were in his starting hand.
  - Alice clues number 1 to Bob, touching two 1's on slots 1 and 3.
  - Bob knows that the correct order to play all of the 1's is is 1, 3.
...
- Most of the time, a _Fix Clue_ is given by retouching the card that needs to be fixed in some way. Rarely, it is also possible to give a _Fix Clue_ by "filling it in" with a negative number clue.
- In the example below:
  - Red 4 and blue 2 are played on the stacks.
  - Before the clue was given, Bob had an unknown blue card in his hand and Cathy had a blue 3 and blue 4 already clued in her hand.
  - Alice know that from _Good Touch Principle_, Bob will conclude that his clued blue card is a blue 5, and he will likely misplay it on a future turn.
  - Alice clues number 5 to Bob, which touches a brand new 5. But it also falsifies the "blue 5" note that Bob has written on his blue card.
  - Bob might be tempted to consider this a _Play Clue_ on the focused 5. (Red 5 is currently playable, so it might be a playable red 5.)
  - However, Bob knows that since the clue prevented a misplay, it is not necessarily a _Play Clue_. Thus, Bob has to keep the 5 in his hand until he gets more information.
