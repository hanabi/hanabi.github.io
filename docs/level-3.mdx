...
import Clue1Stacking from "./level-3/1-clue-stacking.yml";
...
- For example:
  - Alice clues Bob red, touching red 3 on slot 1 and red 1 on chop.
  - Bob plays the chop red card. It successfully plays as red 1. (He then draws a card, and all of the other cards in his hand slide over.) Cathy does something unrelated.
  - Alice clues 1's to Bob, which touches three new card on slot 1, slot 3 and slot 4.
  - Bob knows that fresh 1's have precedence, so he plays the slot 1 card next.
  - After that, Bob plays the 1s from his starting hand from right to left, that is, slot 4 then the slot 3 card.
...
- When this happens, the order of playing the newly clued 1s is calculated separately, and is **stacked before** the order of playing the previously clued 1s.
- The order of playing the newly clued 1s is calculated in a similar fashion - chop 1 followed by fresh 1s.
- For example

  - Alice clues Bob's 1s with a number 1 clue in his starting hand.
  - The order of playing these 1s is from right to left. Bob plays the rightmost 1 (drawing a new card in the process). It successfully plays as red 1.
  - Cathy clues Bob red on the chop card, giving him red 2. Alice does something unrelated.
  - Bob plays the red 2 (drawing another card). Cathy does something unrelated.
  - Alice gives Bob a number 1 clue again, touching Bob's two new cards as well as re-touching the 1s from his starting hand.
  - Bob is surprised to receive two more 1s. He sees that the order of playing the newly clued 1s is slot 2 (Chop-focus exception) then slot 1 (Fresh 1s rule). Bob stacks this order before the old order, now making the order of playing 1s slot 2, slot 1, slot 5, slot 4 and slot 3.
  - Bob also knows that the 1 on slot 3 will never play, so he marks it as known-trash.
...
- A _[superposition](https://en.wikipedia.org/wiki/Quantum_superposition)_ is a term from [quantum mechanics](https://en.wikipedia.org/wiki/Quantum_mechanics) that describes how something can be two different things at the same time. For example, an electron is a type of a particle, and an electron can be "spin up" or "spin down". But an electron could also be in a superposition of both "spin up" and "spin down" at the same time, as weird as that sounds. If an electron happens to bounce off of something (i.e. is measured), then the superposition will "collapse" and it will **only** be in a "spin up" or "spin down" state. The details of quantum mechanics are not important - the point is that we treat unknown Hanabi cards sort of like unknown particles: even though they **really** have a true identity, we ignore that and treat them as being all of the possibilities at the same time.
  - Even though it does not matter for the purposes of Hanabi, you should know that the electron analogy is bad because electrons [do **not** actually have a true identity](https://www.lesswrong.com/posts/9cgBF6BQ2TRB3Hy4E/and-the-winner-is-many-worlds). But just roll with it for now.
...
- For example, in a 3-player game:
  ...
  - Bob writes a _card note_ of "red 2 or blue 2" on his clued card.
