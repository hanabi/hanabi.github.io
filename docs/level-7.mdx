...
- However, if playing the card (player 1) would cause the next player to discard (player 2) and the next player after that (player 3) to have a critical/playable card be on chop that is not clued, it is a very bad situation. Thus, the discard is **just** for the purposes of generating a clue so that player 2 can clue player 3.
- The _Generation Discard_ is **only** to be used as a last resort. If player 3 has something else to do (like play a card or give some obvious clue), then player 1 should just play their card and let player 3's chop card be unclued temporarily.
...
S1("Alice discards with a known-play. (Or, Alice has a known-safe discard and discards her chop instead.)")
...
D2Y([Yes])
D2N([No])
D3[/Does Cathy have a unique playable card or a critical card or a unique 2 on chop?\]
D3Y([Yes])
D3N([No])
D3YL(Alice did a _**Generation Discard**_.<br/ ><br/ >Bob should give a _Play Clue_ or a _Save Clue_ to Cathy's chop card.)
D3NL(Alice did a _**Scream Discard Chop Move**_.<br/ ><br/ >Bob has an important card on his chop, so he should permanently move his chop by one card. Furthermore, he is not allowed to discard on this turn.)
D4[/Does Cathy have a known-play or a known-safe discard?\]
D4Y([Yes])
D4N([No])
D4YL(Alice did an advanced move that is not covered by this flowchart.<br/ ><br/ >_Scream Discards_ and _Generation Discards_ **must** be a last-resort move, so Bob should reason about what cards need to be saved and what players have the safest chops. It is also possible that Alice has made a mistake.)
D5[/Does Cathy have a unique playable card or a critical card or a unique 2 on chop?\]
D5Y([Yes])
D5N([No])
...
- First, see the section on [constructing lines](#lines).
- Normally, when a player has an important card on their chop, one of the other players will make sure that the card is clued before it ever even has a chance of being discarded.
- However, in some rare cases, the team can be very low on clues and cannot reasonably clue a player's important chop card. In these cases, the only thing left to do is to hope that when it gets to their turn, the player decides to give a clue and does not decide to discard.
- Here, we would say that the important card is "riding on chop". This is a metaphor for something perched precariously on the edge of a chopping block, just moments away from being chopped.
- Whenever an important card rides on chop, it is a potentially game losing situation. Thus, you should only construct a line that allows a card to ride on chop as an absolute last resort. Even if you think that the player with the important card will give an "obvious" clue, remember that the unseen cards in your hand (and the drawn cards in the meantime) might influence what they do.
- The [_Generation Discard_](#the-generation-discard) is a basic example of a move that a player can perform to avoid a line with a riding card.

### Blind-Playing Chop Moved Cards

- Oftentimes, cards are _Chop Moved_ towards the beginning of the game. Normally, once the time comes, the _Chop Moved_ card will be directly clued in order to get it to play.
