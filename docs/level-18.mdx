...
import EliminationFinesse1 from "./level-18/elimination-finesse-1.yml";
import EliminationFinesse2 from "./level-18/elimination-finesse-2.yml";
import TrashTouchElimination1 from "./level-18/trash-touch-elimination-1.yml";
...
- One excellent reason that they would let you discard an important card is that you have the other copy in your hand. (In order to satisfy _Good Touch Principle_, they had to wait for you to discard it so that they would not duplicate the card.)
...
- For example, in a 3-player game:
  ...
  - Alice writes a note of "b1" on slot 1.
...
- For example, in a 3-player game:
  ...
  - Normally, Alice would treat this as a _Play Clue_ on the 2 in her slot 1. However, she knows from her _Elimination Notes_ that this is not the red 2, so the focus of the clue moves to the right. Therefore, slot 3 is exactly red 2 (and she can play it right now).
...
- An _Elimination Play Clue_ cannot be a _Delayed Play Clue_, a _Prompt_, or a _Finesse_.
...
- First, see the section on [the definition of riding](level-7.mdx#the-definition-of-riding).
- If:
  - there are only two cards left in a player's hand with _Elimination Notes_ **and**
  - the previous players let the chop card _Ride_ **and**
...
- Note that normal _Finesses_ take priority over _Self-Elimination Finesses_. For example, in a 3-player game:
  ...
  - Bob's hand is completely unclued. Bob has _Elimination Notes_ on slot 3, 4 and 5 for blue 3.
  - Alice clues a number 4 in Bob's slot 1 as a _Self-Finesse_.
  - If Bob's 4 is a red 4, then this would be a normal _Self-Finesse_, and Bob should blind-play red 3 from slot 2.
  - If Bob's 4 is a blue 4, then this would be an _Elimination Self-Finesse_, and Bob should blind-play blue 3 from slot 5.
  - Bob knows that in this situation, you always supposed to assume a normal _Finesse_, so he blind-plays red 3 from slot 2.
...
- For example, in a 3-player game:
  ...
  - Cathy's hand contains a red 5, unbeknownst to her.
  - Alice wants to give a _Play Clue_ to the red 4. Everyone else on the team (other than Bob) can see that she can give either number 4 or red.
...
- First, see the section on the _[Trash Chop Move](level-4.mdx#the-trash-chop-move-tcm)_.
...
- For example, in a 3-player game:
  ...
  - Bob's hand is completely unclued and is, from newest to oldest: `blue 1, blue 1, red 4, blue 3, green 3`.
  - Cathy's hand contains a red 5, unbeknownst to her.
