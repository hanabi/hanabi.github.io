...
- However, in a hypothetical where Bob's 2 turned out to be a red 2, Bob's move move would be a _Gentleman's Discard_. But slot 2, slot 3, slot 4, and slot 5 would still be _Chop Moved_ in the same way (even though cards are not normally _Chop Moved_ from a _Gentleman's Discard_).
