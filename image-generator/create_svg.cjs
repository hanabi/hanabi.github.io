...
class Imager {
	...
	constructor(yaml_file) {
		...
		this.#all_suits = (yaml_file.get('stacks') ?? NO_VARIANT_SUITS.map(a => new Map([[a, 0]]))).map(a => a.keys().next().value);
		...
		this.#svg_file.attributes.set('viewBox', `0 ${this.#y_top} ${this.#x_max} ${y_max - this.#y_top}`);
	}
...
if (!this.#yaml_file.has('stacks')) {
		...
		for (const color_value of this.#yaml_file.get('stacks')) {
			...
			this.#svg_file.addImage(`${PIECES_PATH}/cards/${file_name}.svg`, {x: x_offset, y: 0, width: CARD_WIDTH, height: CARD_HEIGHT});
	...
	#draw_player_rows() {
		this.#yaml_file.get('players').forEach((player, player_num) => {
			if (player.has('text')) {
				...
				this.#svg_file.addText(player.get('text'), {x: this.#x_offset_where_player_begins + 40, y: this.#y_offset, dy: 20, 'class': TEXT_COLOR_CLASS});
				this.#y_offset += 50;
			} else {
				// Draw a row representing a player's hand.
				const name = this.#draw_player_name(player_num, player);
				const four_or_more_players = name == PLAYER_NAMES[3] || name == PLAYER_NAMES[4];
				...
				this.#x_offset = this.#x_offset_where_player_begins + HORIZONTAL_SPACING_BETWEEN_PLAYER_NAME_AND_HAND;
				...
				this.#y_offset += CARD_HEIGHT + this.#y_below + VERTICAL_SPACING_BETWEEN_PLAYERS;
	...
	#draw_player_name(player_num, player) {
		...
		const r = this.#svg_file.addSvg({x: this.#x_offset_where_player_begins, y: this.#y_offset, width: HORIZONTAL_SPACING_BETWEEN_PLAYER_NAME_AND_HAND, height: CARD_HEIGHT});
		r.addText(name, {x: '0%', y: '50%', 'class': TEXT_COLOR_CLASS, style: 'font-size: 1.4em;', 'dominant-baseline': 'central'});

		if (player.has('clue_giver')) {
			// Based on: https://stackoverflow.com/a/42783381/14347173
			const clue_giver_description = "Clue Giver";
			r.addText(clue_giver_description, {x: '0%', y: '50%', dy: 30, fill: 'black', 'dominant-baseline': 'central', filter: 'url(#clue_giver)'});
			r.addText(clue_giver_description, {x: '0%', y: '50%', dy: 30, fill: 'black', 'dominant-baseline': 'central'});
		}
		return name;
	}
	...
	#draw_player_card(card) {
		...
		if (clued) {
			this.#draw_clued_card(card_type, crossed_out, orange, this.#x_offset, this.#y_offset);
} else {
	...
	#draw_clue_border() {
		...
		this.#svg_file.addRect({x: this.#x_offset - (clue_border_overlap / 2), y: this.#y_offset - (clue_border_overlap / 2), width: CARD_WIDTH + clue_border_overlap, height: CARD_HEIGHT + clue_border_overlap, fill: CLUE_BORDER_COLOR, rx: CARD_ROUNDED_CORNER_SIZE, ry: CARD_ROUNDED_CORNER_SIZE });
	...
	#draw_unclued_card(pips, crossed_out, orange) {
		...
		const s = this.#svg_file.addSvg({x: this.#x_offset, y: this.#y_offset, width: CARD_WIDTH, height: CARD_HEIGHT});
		s.addRect({x: 0, y: 0, width: CARD_WIDTH, height: CARD_HEIGHT, fill: 'gray', rx: CARD_ROUNDED_CORNER_SIZE, ry: CARD_ROUNDED_CORNER_SIZE});
	...
	#draw_clued_card(card_type, crossed_out, orange, x, y) {
		...
		if (ranks.size !== 1 && suits.size !== 1) {
			...
			const s = this.#svg_file.addSvg({x, y, width: CARD_WIDTH, height: CARD_HEIGHT});
			s.addRect({x: 0, y: 0, width: CARD_WIDTH, height: CARD_HEIGHT, fill: 'gray', rx: CARD_ROUNDED_CORNER_SIZE, ry: CARD_ROUNDED_CORNER_SIZE});
			...
			const s = this.#svg_file.addSvg({x, y, width: CARD_WIDTH, height: CARD_HEIGHT});
			s.addImage(`${PIECES_PATH}/cards/${ranks.keys().next().value}.svg`, {x: 0, y: 0, width: CARD_WIDTH, height: CARD_HEIGHT});
			this.#draw_card_pips(s, suits, crossed_out, orange);
		} else if (ranks.size !== 1 && suits.size === 1) {
			// This is a card with a known color and an unknown rank.
			const s = this.#svg_file.addSvg({x, y, width: CARD_WIDTH, height: CARD_HEIGHT});
			s.addImage(`${PIECES_PATH}/cards/${this.#suit_filenames.get(suits.keys().next().value)}.svg`, {x: 0, y: 0, width: CARD_WIDTH, height: CARD_HEIGHT});
			this.#draw_card_pips(s, ranks, crossed_out, orange);
		} else {
			// An exact card identity was specified. (e.g. "r1")
			this.#svg_file.addImage(`${PIECES_PATH}/cards/${this.#suit_filenames.get(suits.keys().next().value)}${ranks.keys().next().value}.svg`, {x, y, width: CARD_WIDTH, height: CARD_HEIGHT});
		}
	}
	...
	#validate_card_type(card) {
		...
		if (letters.length >= 2) {
			const sorted_letters = JSON.parse(JSON.stringify(letters)).sort((a, b) => {
				const aa = this.#all_suits.indexOf(a);
				const bb = this.#all_suits.indexOf(b);
				return aa - bb;
			}).join('');
			if (letters.join('') !== sorted_letters) {
				throw new Error(`When defining a card, the suits must be specified in order from left to right. In other words, "${letters.join('')}" should be "${sorted_letters}".`);
			}
		}
	}
	...
	#draw_card_pips(svg, pips, crossed_out, orange) {
		...
		for (let rank = 1; rank < 6; ++rank) {
			if (pips.has(rank + '')) {
				const rect = svg.addSvg({x: (rank - 1) * rank_pip_width, y: CARD_HEIGHT * 4 / 5, width: rank_pip_width, height: CARD_HEIGHT / 5});
				rect.addText(rank, {x:'50%', y:'50%', fill: orange.has(rank + '') ? 'orange' : 'white', style: 'filter: url(#shadow_rank)', 'text-anchor': 'middle', 'dominant-baseline': 'central'});
				if (crossed_out.has(rank + '')) {
					rect.addImage(`${PIECES_PATH}/x.png`, {x:CARD_WIDTH / 10 - 6, y:CARD_HEIGHT / 10 - 6, width:12, height:12, style:'filter: url(#black_x_rank)'});
				}
			}
		}
		...
		this.#all_suits.forEach((color, i) => {
			if (pips.has(color)) {
				svg.addImage(`${PIECES_PATH}/pips/${this.#suit_filenames.get(color)}.svg`, {x: CARD_WIDTH / 2 - 6 - 20 * Math.sin(angle * i), y: CARD_HEIGHT / 2 - 6 - 20 * Math.cos(angle * i), width: 12, height: 12, style: 'filter: url(#shadow_suit)'});
				if (crossed_out.has(color)) {
				svg.addImage(`${PIECES_PATH}/x.png`, {x: CARD_WIDTH / 2 - 6 - 20 * Math.sin(angle * i) - 2, y: CARD_HEIGHT / 2 - 6 - 20 * Math.cos(angle * i) - 2, width: 16, height: 16, style: 'filter: url(#black_x_suit)'});
				}
			}
		});
	}
	...
	#draw_extra_card_attributes(card) {
		if (card.get('trash')) {
			this.#svg_file.addImage(`${PIECES_PATH}/trashcan.png`, {x:this.#x_offset+5, y:this.#y_offset+5, width:CARD_WIDTH - 10, height:CARD_HEIGHT - 10,opacity:0.4});
		}

		if (card.has('clue')) {
			// Draw the arrow above the card.
			const arrow_name = card.get('retouched') ? 'arrow_dark' : 'arrow';
			this.#svg_file.addImage(`${PIECES_PATH}/${arrow_name}.svg`, {x:this.#x_offset+10, y:this.#y_offset-40, width:50, height:70});
			...
			this.#svg_file.addElement('circle', {cx:this.#x_offset+35,cy:this.#y_offset-15, r:15, fill:color, stroke:color==='black' ? 'white' : 'black', 'stroke-width':2});

			if (/^\d$/.test(card.get('clue'))) {
				const r = this.#svg_file.addSvg({x: this.#x_offset + 27, y: this.#y_offset - 23, width: 16, height: 16});
				r.addText(card.get('clue'), {x:'50%', y:'50%',fill:'white',style:'font-size: 1.4em;', 'text-anchor':'middle','dominant-baseline':'central'});
			}
		...
		if (card.get('middle_note')) {
			...
			const r = this.#svg_file.addSvg({x: this.#x_offset, y: this.#y_offset, width: CARD_WIDTH, height: CARD_HEIGHT});
			r.addText(card.get('middle_note'), {x:'50%', y:'50%', fill:color, stroke:color, style:'font-size: 1.5em; filter: url(#shadow_rank);', 'text-anchor': 'middle', 'dominant-baseline':'central'});
		}
	}
	...
	#draw_textbox(opts, offset) {
		...
		if (text[0].startsWith('Rainbow')) {
			width = 85;
			r = this.#svg_file.addSvg({x: this.#x_offset - 10, y: this.#y_offset + offset, width, height: 20 * text.length});
		} else {
			width = 64;
			r = this.#svg_file.addSvg({x: this.#x_offset + 3, y: this.#y_offset + offset, width, height: 20 * text.length});
		}
		const text_color = new Set(["gold", "yellow", "rainbow", "pink"]).has(color) ? 'black' : 'white';

		r.addRect({x: 0, y: 0, width, height: 20 * text.length, stroke: text_color, fill: color === 'rainbow' ? 'url(#rainbowtext)' : color});

		text.forEach((line, i) => {
			r.addSvg({x: 0, y: 20 * i, width, height: 20}).addText(line, {x:'50%', y:'50%', fill:text_color, 'text-anchor':'middle', 'dominant-baseline':'central'});
		});
	...
	#draw_big_text() {
		...
		const x_of_text = (this.#all_suits.length * (CARD_WIDTH + HORIZONTAL_SPACING_BETWEEN_CARDS) - HORIZONTAL_SPACING_BETWEEN_CARDS - TEXT_WIDTH) / 2;
		...
		const text_color = new Set(["gold", "yellow", "rainbow"]).has(color) ? 'black' : 'white';

		const r = this.#svg_file.addSvg({x: x_of_text, y: y_of_text, width: TEXT_WIDTH, height: TEXT_HEIGHT});
		r.addRect({x: 0, y: 0, width: TEXT_WIDTH, height: TEXT_HEIGHT, stroke: text_color, fill: color});
		r.addText(text, {x:'50%', y:'50%', fill:text_color, 'text-anchor':'middle', 'dominant-baseline':'central', style:'font-size: 2em;'});
	...
	#draw_discard_pile() {
		...
		const x_of_discard_pile = (this.#all_suits.length * (CARD_WIDTH + HORIZONTAL_SPACING_BETWEEN_CARDS) - HORIZONTAL_SPACING_BETWEEN_CARDS - TRASH_WIDTH) / 2;
		const y_of_discard_pile = this.#left_y_offset;

		this.#svg_file.addImage(`${PIECES_PATH}/trashcan.png`, {x: x_of_discard_pile, y: y_of_discard_pile, width: TRASH_WIDTH, height: TRASH_HEIGHT, opacity: 0.2});

		const width_total = CARD_WIDTH + (discarded.length-1)*CARD_WIDTH/2;
		const height_total = CARD_HEIGHT + (discarded.length - 1) * CARD_HEIGHT / 3;
		...
		this.#svg_file.addRect({x: x_of_discard_pile, y: y_of_discard_pile, width: TRASH_WIDTH, height: TRASH_HEIGHT, rx: CARD_ROUNDED_CORNER_SIZE * 2, ry: CARD_ROUNDED_CORNER_SIZE * 2, stroke: 'darkgreen', fill: 'none', 'stroke-width': 2});
