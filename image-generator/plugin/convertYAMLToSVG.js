...
class ImageGenerator {
    ...
    constructor(yamlFile) {
        ...
        this.#all_suits = (yamlFile.get("stacks") ?? NO_VARIANT_SUITS.map((a) => new Map([[a, 0]]))).map((a) => a.keys().next().value);
    ...
    #draw_card_pips(svg, pips, crossed_out, orange) {
        ...
        this.#all_suits.forEach((color, i) => {
            if (pips.has(color)) {
                svg.addImage(`${PIECES_PATH}/pips/${this.#suit_filenames.get(color)}.svg`, {
    ...
    #draw_big_text() {
        ...
        const x_of_text = (this.#all_suits.length *
    ...
    #draw_discard_pile() {
        ...
        const x_of_discard_pile = (this.#all_suits.length *
            (CARD_WIDTH + HORIZONTAL_SPACING_BETWEEN_CARDS) -
            HORIZONTAL_SPACING_BETWEEN_CARDS -
            TRASH_WIDTH) /
            2;
...
function convertYAMLToSVG(source) {
    const yamlFile = yaml_1.default.parse(source, {
        mapAsMap: true,
    });
